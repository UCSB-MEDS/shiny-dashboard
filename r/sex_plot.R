sex_plot <- function(input) {
  ...
  sex_program_time <- enrolled %>% 
    select(c("app_submission_year", "application_id", "gender", "objective1")) %>% 
    group_by(app_submission_year, objective1, gender) %>% 
    summarize(count = n())
  
  sex_stats_time <- left_join(sex_program_time, program_size,
                              by = c("app_submission_year", "objective1")) %>% 
  ...
  renderPlotly({
    ...
    sex_all <- ggplot(data = sex_stats_time,
                      aes(x = app_submission_year, y = percent, fill = reorder(gender, percent),
                          text = paste0(gender, " (", percent, "%", ")", "\n", "Sample size: ", size))) +
      geom_bar(position = "dodge", stat = "identity") +
      scale_x_continuous(breaks = seq(min(sex_stats_time$app_submission_year),
                                      max(sex_stats_time$app_submission_year))) +
