admissions_plot <- function(input) {
  ...
  admissions_stacked_all <- reactive({
    ...
    admissions %>% 
      select(c(ay_year, objective1, Enrolled, Applied, Admitted)) %>% 
  ...
  renderPlotly({
    ...
    admissions_all_plot <- ggplot(data = admissions_stacked_all(), aes(x = ay_year, y = counts, fill = reorder(admin_tots, counts))) +
